...
public class HearthstoneItem extends Item implements IClickable {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if (pos.getY() > 0 && storedDim != null){
            ...
            tooltip.add(Util.tooltipStyle("tooltip.principium.hearthstone.set_block_pos", pos.getX(), pos.getY(), pos.getZ(), dimString));
        } else {
            tooltip.add(Util.tooltipStyle("tooltip.principium.hearthstone.no_set_block_pos"));
        }

        tooltip.add(Util.loreStyle("lore.principium.hearthstone"));
        ...
        if (world.getDimension().getType() != dim){
            if (livingEntity instanceof PlayerEntity){
                ((PlayerEntity)livingEntity).sendStatusMessage(
                        new TranslationTextComponent("status.principium.hearthstone.wrong_dimension"), true
                );
            }
        ...
        if (context.isPlacerSneaking()) {
            ...
            if (context.getPlayer() != null){
                context.getPlayer().sendStatusMessage(new TranslationTextComponent(
                        "status.principium.hearthstone.set_block_pos",
                        context.getPos().getX(),
                        context.getPos().getY(),
                        context.getPos().getZ()
                ), true);
    ...
    public boolean hitEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (target instanceof PlayerEntity && !target.isSneaking()){
            ((PlayerEntity) target).sendStatusMessage(new TranslationTextComponent("status.principium.hearthstone.player_not_sneaking"), true);
        } else {
            BlockPos pos = getStoredPos(stack);
